...
const inventoryAnalyticsAPI = {
  ...
  getStockoutRisk: async () => {
    const response = await api.get('/api/v1/analytics/inventory/stockout-risk/');
    return response.data;
  },

  // Get at-risk products
  getAtRiskProducts: async () => {
    const response = await api.get('/api/v1/analytics/inventory/at-risk/');
    return response.data;
  },
  ...
  getMovements: async (params = {}) => {
    ...
    const response = await api.get(`/api/v1/analytics/inventory/movements/?${queryParams.toString()}`);
    return response.data;
  },
  ...
  getDashboardStats: async (params = {}) => {
    ...
    const response = await api.get(`/api/v1/analytics/inventory/dashboard-stats/?${queryParams.toString()}`);
    return response.data;
  }
};
