...
const healthcareAnalyticsAPI = {
  ...
  // Get lab stage timing (average hours per lifecycle stage)
  getLabStageTiming: async (params = {}) => {
    const queryParams = new URLSearchParams();
    const startDate = formatDate(params.start_date);
    const endDate = formatDate(params.end_date);
    if (startDate) queryParams.append('start_date', startDate);
    ...
    const response = await api.get(`/analytics/healthcare/lab-stage-timing/?${queryParams.toString()}`);
    return response.data;
  },

  // Get prescriber analytics (revenue, commissions, patient counts)
  getPrescriberAnalytics: async (params = {}) => {
    const queryParams = new URLSearchParams();
    if (params.period) queryParams.append('period', params.period);
    const startDate = formatDate(params.start_date);
    const endDate = formatDate(params.end_date);
    if (startDate) queryParams.append('start_date', startDate);
    if (endDate) queryParams.append('end_date', endDate);
    if (params.prescriber_id) queryParams.append('prescriber_id', params.prescriber_id);
