...
def supplier_analysis(request):
    ...
    diversity_stats = {
        'local_suppliers': Supplier.objects.filter(is_local=True, status='active').count(),
        'minority_owned': Supplier.objects.filter(is_minority_owned=True, status='active').count(),
        'woman_owned': Supplier.objects.filter(is_woman_owned=True, status='active').count(),
        'indigenous': Supplier.objects.filter(is_indigenous=True, status='active').count(),
        'total_active': Supplier.objects.filter(status='active').count(),
    }
